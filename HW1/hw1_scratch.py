...
def main():
    ...
    config = BertConfig(
        ...
        hidden_size=256,                  # smaller hidden size (default BERT-base = 768)
        num_hidden_layers=4,              # fewer layers (default BERT-base = 12)
        num_attention_heads=4,            # fewer heads (default BERT-base = 12)
        intermediate_size=512,            # smaller feed-forward dimension
    )
    ...
    model = BertForMultipleChoice(config)  # randomly initialized weights
    ...
    if training_args.do_predict:
        ...
        output_path = os.path.join(training_args.output_dir, "test_predictions.json")
