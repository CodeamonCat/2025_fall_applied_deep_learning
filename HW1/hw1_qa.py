...
class DataTrainingArguments:
    ...
    test_output: Optional[str] = field(
        default=None,
        metadata={
            "help": "The output file name for test set predictions (a json file)."
        },
    )
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        context_texts = [contexts[x] for x in examples["relevant"]]

        tokenized_examples = tokenizer(
            examples[question_column_name] if pad_on_right else context_texts,
            context_texts if pad_on_right else examples[question_column_name],
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            # Normalize answer structure to lists and support both 'start' and 'answer_start'.
            if answers is None:
                start_list, text_list = [], []
            else:
                if "answer_start" in answers:
                    start_field = answers["answer_start"]
                else:
                    start_field = answers.get("start")

                text_field = answers.get("text")

                # Convert scalars to lists
                if isinstance(start_field, list) or start_field is None:
                    start_list = start_field or []
                else:
                    start_list = [start_field]

                if isinstance(text_field, list) or text_field is None:
                    text_list = text_field or []
                else:
                    text_list = [text_field]

            has_answer = (
                len(start_list) > 0 and len(text_list) > 0 and text_list[0] is not None
            )
            ...
            if not has_answer:
                ...
                start_char = start_list[0]
                end_char = start_char + len(text_list[0])
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        # Normalize answers to always have 'answer_start'
        references = []
        for ex in examples:
            ans = ex[answer_column_name]
            if "answer_start" not in ans and "start" in ans:
                ans = {
                    "text": ans.get("text", []),
                    "answer_start": ans.get("start", []),
                }

            text_field = ans.get("text", [])
            start_field = ans.get("answer_start", [])

            if isinstance(text_field, str):
                text_field = [text_field]
            if isinstance(start_field, int):
                start_field = [start_field]

            references.append(
                {
                    "id": str(ex["id"]),
                    "answers": {"text": text_field, "answer_start": start_field},
                }
            )
    ...
    if training_args.do_predict:
        ...
        # Convert predictions to DataFrame and save as CSV with columns 'id' and 'answer'
        df = pd.DataFrame(results.predictions)
        df = df.rename(columns={"prediction_text": "answer"})
        output_file = data_args.test_output if data_args.test_output else os.path.join(training_args.output_dir, "prediction.csv")
        df.to_csv(output_file, index=False)
        logger.info(
            f"Predictions saved to {os.path.join(training_args.output_dir, 'prediction.csv')}"
        )
